...
### inaddrレジスタ
　input_portの入力先アドレスを指定します。あるアドレスからデータを入力する場合は、inaddrレジスタに指定のアドレスを格納した後、input_portからデータを取り込むという手順を取ることになります。
### outaddrレジスタ
　output_portの出力先アドレスを指定します。あるアドレスへデータを出力する場合は、outaddrレジスタに指定のアドレスを格納した後、output_portにデータを格納するという手順を取ることになります。
### bufferレジスタ
　外部から送信されたデータを一時的に格納します。実体はメモリです。buffer_pointerレジスタの値番地のデータをCPUに出力します。現時点の設計においては、bufferレジスタに書き込みを行うことはできず、input_port経由でしか書き込みを行うことは出来ません。
### buffer_pointerレジスタ
　bufferレジスタのアドレス指定を担当するレジスタです。buffer_pointerレジスタが指し示しているアドレスに外部から入力されたデータの読み書きを行います。
### buffer_sizeレジスタ
　buffer_sizeレジスタに格納された値だけデータの読み込みが行われた場合（ここでの値はバイト数ではない。16bitを1とする単位、つまり読み込みバイト数を2で割った値である）、割り込み信号を発出します。
### INTNUMレジスタ
...
|5|読み込みバースト転送モードフラグ。読み込みバースト転送モードフラグが1であるときinaddrへの書き込みがすべて無効になります。
|6|実行モードフラグ。0はカーネルモード、1はユーザーモードであることを表します。
|[16:7]|未使用|
