...
### INTNUMレジスタ
　割り込み種別を格納するレジスタです。NC-16に割り込み信号を入力する際、割り込み種別（どの外部機器から割り込み信号が発せられたか、どのような内部割込みが発生したのか）を指定する必要があります。割り込み信号を入力する前にINTNUMレジスタに割り込み種別を意味する値を格納しておくと、その割り込み種別に応じた処理が実行されます。
### フラグレジスタ(FLAGS)
