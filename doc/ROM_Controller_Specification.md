...
## ピンアサイン
- data_in：NC-16からの命令を受信します。ビット幅は16bitです。
- input_strobe：読み込みストローブ信号です。data_inのデータが有効であるとき、1を入力し、無効であるときは0を入力してください。
- RESET：リセット信号を入力してください。
- CLK：クロックを入力します。NC-16に入力するクロックと同じクロック信号を入力してください。
- output_memaddr：ROMアドレスを出力します。ROMのアドレス端子に接続してください。
- write_probe：書き込みプローブ信号です。ROMから出力されるデータが有効であるとき1を出力します。無効であるときは0を出力します。
## 機能概要
