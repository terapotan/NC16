...
4. 3ステージ目以降：Microcode ROMに格納されているマイクロコードを実行する。終端ステージ数に達した場合は、IDSCにリセット信号を送出し、0ステージ目に戻る。実行フェーズは終了し命令フェッチのフェーズに戻る。
...
### Instruction Terminate Stage Decoder
...
　Instruction Decoderから出力するオペランドを、IRに格納されているオペランドとするか、MORに格納されているオペランドとするか選択できます。0b1のときIRに格納されているオペランドを、0b0のときMORに格納されているオペランドを出力します。
