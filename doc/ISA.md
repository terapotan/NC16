...
## syscall
　実行モードをカーネルモードにした上で、system call addressへジャンプし、OSのシステムコールを呼び出します。system call addressについては[メモリマップ](./Memory_map.md)を参照してください。
|ニーモニック|ファンクションコード|FuncInput1|FuncInput2|オペランド|動作
|:--:|:--:|:--:|:--:|:--:|:--:|
|syscall|0x27|0xf|0xf|0x0000|実行モードをカーネルモードにし、システムコールを呼び出します
## usercall
...
## sysret
　システムコール呼出しからユーザープログラム側に処理を復帰させます。
|ニーモニック|ファンクションコード|FuncInput1|FuncInput2|オペランド|動作
|:--:|:--:|:--:|:--:|:--:|:--:|
|sysret|0x2a|0xf|0xf|0x0000|システムコール呼出しからユーザープログラム側に処理を復帰させます
## スタック操作系命令
### push
...
|ret|call命令の呼び出し元に戻る|
## 無効命令
　オペコード``0xff``はシステムによって予約されています。将来にわたってこのオペコードが意味のある命令として使われることはなく、またユーザーが意味のあるオペコードとして使用してもいけません。本オペコードがシステムに入力された場合の動作は未定義命令がシステムに入力された場合の挙動と同じです。
## 未定義命令
